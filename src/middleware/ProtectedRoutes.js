...
export default function ProtectedRoutes({ children }) {
	...
	if (role && token && (path.includes("login") || path.includes("register"))) redirect("/landing");
	if (!(path.includes("company_dashboard") || path.includes("userdashboard") || path.includes("admindashboard") || commonRoute))
		redirect("/not-found");
	if (commonRoute) return <>{children}</>;
	if (
		!(companyRoute || empolyeeRoute || adminRoute) &&
		(path.includes("company_dashboard") || path.includes("userdashboard") || path.includes("admindashboard"))
	) {
		redirect("/landing");
	} else
	return <>{children}</>;
